...
object Approximate {
  def main(args: Array[String]) : Unit = {
    ...
    val measurements = (1 to scala.math.max(1,conf.num_measurements()))
      .map(_ => timingInMs( () => {
        mae_exact(train,test,conf_k,sc)
    }))
    ...
    conf.json.toOption match {
      ...
      case Some(jsonFile) => {
        val answers = ujson.Obj(
          ...
          "AK.1" -> ujson.Obj(
            "knn_u1v1" -> ujson.Num(approximateKNNSimilarity(train,partitionedUsers, conf_k, sc)(1,1)),
            "knn_u1v864" -> ujson.Num(approximateKNNSimilarity(train,partitionedUsers, conf_k, sc)(1,864)),
            "knn_u1v344" -> ujson.Num(approximateKNNSimilarity(train,partitionedUsers, conf_k, sc)(1,344)),
            "knn_u1v16" -> ujson.Num(approximateKNNSimilarity(train,partitionedUsers, conf_k, sc)(1,16)),
            "knn_u1v334" -> ujson.Num(approximateKNNSimilarity(train,partitionedUsers, conf_k, sc)(1,334)),
            "knn_u1v2" -> ujson.Num(approximateKNNSimilarity(train,partitionedUsers, conf_k, sc)(1,2))
          ),
          "AK.2" -> ujson.Obj(
            "mae" -> ujson.Num(mae) 
          ),
